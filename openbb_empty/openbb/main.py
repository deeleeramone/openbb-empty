...
routes = [p for p in openapi["paths"] if p.startswith("/api") and "get" in openapi["paths"][p]]
for route in routes:
    ...
    if data_schema and "properties" in data_schema and "results" in data_schema["properties"]:
...
def launch_api():
    ...
    if pat:
        ...
        if hub_credentials:
            ...
            persist_input = (
                input("\n\nDo you want to persist the new settings?" + " Not recommended for public machines. (yes/no): ")
                .strip()
                .lower()
            )
    ...
    try:
        uvicorn.run("openbb.main:app", host=host, port=port, reload=True)
    finally:
