...
def get_query_schema_for_widget(openapi_json: dict, command_route: str) -> tuple[dict, bool]:
    ...
    for param in command_schema.get("parameters", []):
        if param["in"] == "query":
            ...
            if param_name == "provider" and "enum" in param["schema"] and len(param["schema"]["enum"]) == 1:
            ...
            # Enum within anyOf
                ...
                if enums:  # If any enums were found, remove duplicates
                    ...
                    types = [sub_schema.get("type") for sub_schema in param["schema"]["anyOf"] if "type" in sub_schema]
            ...
            if param_name == "start_date":
                ...
                _query_schema["optional"]["start_date"] = (datetime.now() - timedelta(days=90)).strftime("%Y-%m-%d")
...
def data_schema_to_columns_defs(openapi_json, result_schema_ref):
    ...
    if "anyOf" in result_schema_ref:
        for item in result_schema_ref["anyOf"]:
            ...
            if "items" in item and "oneOf" in item["items"]:
                ...
                schema_refs.extend(
                    [oneOf_item["$ref"].split("/")[-1] for oneOf_item in item["items"]["oneOf"] if "$ref" in oneOf_item]
                )
    ...
    schemas = [
        openapi_json["components"]["schemas"][ref] for ref in schema_refs if ref in openapi_json["components"]["schemas"]
    ]
    ...
    for key in common_keys:
        ...
        if "anyOf" in prop:
            types = [sub_prop.get("type") for sub_prop in prop["anyOf"] if "type" in sub_prop]
            if "number" in types or "integer" in types or "float" in types:
                cell_data_type = "number"
            elif "string" in types and any(
                sub_prop.get("format") in ["date", "date-time"] for sub_prop in prop["anyOf"] if "format" in sub_prop
            ):
                cell_data_type = "date"
            else:
                cell_data_type = "text"
        ...
        column_def["chartDataType"] = "series" if cell_data_type in ["number", "integer", "float"] else "category"
